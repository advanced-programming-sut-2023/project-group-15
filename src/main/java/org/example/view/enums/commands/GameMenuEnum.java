...
public enum GameMenuEnum {
    ...
    MOVE_UNIT("^\\s*move\\s+unit\\s+((-x\\s*(?<x>\\d+)\\s*)|(-y\\s*(?<y>\\d+)\\s*))*$"),
    DIG_TUNNEL("^\\s*dig\\s+tunnel\\s+((-x\\s*(?<x>\\d+)\\s*)|(-y\\s*(?<y>\\d+)\\s*))*$"),
    SET_STATE("^\\s*set\\s+((-x\\s*(?<x>\\d+)\\s*)|(-y\\s*(?<y>\\d+)\\s*)|(-s\\s+(?<state>standing|offensive|defensive)\\s*))*$"),
    AIR_ATTACK("^\\s*attack\\s+((-x\\s*(?<x>\\d+)\\s*)|(-y\\s*(?<y>\\d+)\\s*))*$"),
    POUR_OIL("^\\s*pour\\s+oil\\s+-d\\s+(?<direction>up|down|right|left)\\s*$"),
    DISBAND_UNIT("^\\s*disband\\s+unit\\s*$"),
    DIG_DITCH("^\\s*dig\\s+ditch\\s+((-x\\s*(?<x>\\d+)\\s*)|(-y\\s*(?<y>\\d+)\\s*))*$"),
    PATROL_UNIT("^\\s*patrol\\s+unit\\s+((-x1\\s+(?<x1>\\d+)\\s*)|(-x2\\s+(?<x2>\\d+)\\s*)|(-y1\\s+(?<y1>\\d+)\\s*)|(-y2\\s+(?<y2>\\d+)\\s*))*$"),
    ...
    SELL_ITEM("^\\s*sell\\s+((-i\\s*(?<item>\\S+)\\s*)|(-a\\s*(?<number>)\\d+)\\s*)*$"),
    BUY_ITEM("^\\s*buy\\s+((-i\\s*(?<item>\\S+)\\s*)|(-a\\s*(?<number>)\\d+)\\s*)*$"),
